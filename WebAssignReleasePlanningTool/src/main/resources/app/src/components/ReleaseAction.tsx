...
export default function ReleaseAction(props: {id: null | String, type: String}) {
    const [title, setTitle] = useState("");
    const [duration, setDuration] = useState("");
    const [repository, setRepository] = useState({name: "", id: ""} as Repository);
    const [action, setAction] = useState("PULL_REQUEST");
    ...
    return(<>
        ...
        <Flex behavior={FlexBehavior.container} spacing={3} style={{padding:"40px"}}>
            ...
            { props.type == "repositoryAction" && <>
            <Flex behavior={FlexBehavior.item} xs={2}>
                    <Input 
                        labelText="Repository:" 
                        value={repository.id} 
                        type={InputType.text} 
                        onChange={(e)=> setRepository({...repository, name: e.currentTarget.value})}
                    />
            </Flex>  
            <Flex behavior={FlexBehavior.item} xs={2}>
                    <Input labelText="Action:" value={action} type={InputType.text} onChange={(e)=>{
